...
const HacerEjercicio = () => {
  ...
  const [ejercicioStats, setEjercicioStats] = useState({});
  ...
  useEffect(() => {
    if (typed.length > 0 && typed.length === codigo2.length) {
      ...
      if (ejercicioStats) {
        const ejercicioStatsObject = typeof ejercicioStats === 'object' ? ejercicioStats : JSON.parse(ejercicioStats);
  ...
  const updateEjercicioStats = async (stats) => {
    try {
      console.log("stats", stats);
      await axios.put(`${endpoint}/ejercicioStatsDocente/${id}`, {
      statsDocente: JSON.stringify(stats),
      });
      navigate('/mostrarEjercicio');
    } catch (error) {
